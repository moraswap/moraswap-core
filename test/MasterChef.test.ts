...
describe("MasterChef", function () {
    ...
    context("With ERC/LP token added to the field", function () {
        beforeEach(async function () {
            ...
            this.bonusReward = await this.ERC20Mock.deploy("BonusToken", "BONUS", "10000000000")
        })
        ...
        it("should give proper MORAs after updating emission rate", async function () {
            ...
            await this.mora.transferOwnership(this.chef.address) // t-56
            ...
            await this.chef.addPool("10", this.lp.address, this.rewarder.address, false) // t-53
            ...
            // At t+110,
            // Alice should have:   100*100*0.9 = 9000 (+90) MORA
            ...
            // At t+115,
            // Alice should have:   9000 + 1*100*0.9 + 4*40*0.9 = 9234 (+36) MORA
            //                      1000 + 1*10 + 4*10 = 1050 (+10) BONUS
            await advanceTimeAndBlock(4) // t+115
            expect((await this.chef.pendingReward(0, this.alice.address)).pendingMora).to.be.within(9234, 9270)
        ...
        it("should not allow same LP token to be added twice", async function () {
            ...
            await this.chef.addPool("100", this.lp.address, "0x0000000000000000000000000000000000000000", false)
        ...
        it("should allow a given pool's allocation weight to be updated", async function () {
            ...
            await this.chef.setPool("0", "150", "0x0000000000000000000000000000000000000000", true)
        ...
        it("should give out MORAs only after farming time", async function () {
            ...
            await this.mora.burn("500000000000000000000000000") // t-58 // burn to test easier with small number
            await this.mora.transferOwnership(this.chef.address) // t-57
            await this.chef.setPercent(this.burnPercent) // t-56

            await this.chef.addPool("100", this.lp.address, "0x0000000000000000000000000000000000000000", false) // t-55
            ...
            expect(await this.mora.totalSupply()).to.be.within(1500, 1600)
        })
        ...
        it("should not distribute MORAs if no one deposit", async function () {
            ...
            expect(await this.mora.totalSupply()).to.equal("0")
            ...
            expect(await this.mora.totalSupply()).to.be.within(1200, 1300)
        ...
        it("should distribute MORAs properly for each staker", async function () {
            ...
            await this.mora.burn("500000000000000000000000000") // t-56 // burn to test easier with small number
            await this.mora.transferOwnership(this.chef.address) // t-55
            await this.chef.setPercent(this.burnPercent) // t-54

            await this.chef.addPool("100", this.lp.address, this.rewarder.address, false) // t-53
            ...
            //   Alice should have: 4*100*0.9 + 4*1/3*100*0.9 + 2*1/6*100*0.9 = 510 (+90) MORA
            //                      4*10 + 4*1/3*10 + 2*1/6*10 = 56.66 (+10) BONUS
            //   MasterChef should have: 1000 - 510 - 100 = 390 (+100) MORA
            await advanceTimeAndBlock(1) // t+19
            await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address }) // t+20
            expect(await this.mora.totalSupply()).to.be.within(1000, 1100)
            ...
            //   Bob should have:   4*2/3*100*0.9 + 2*2/6*100*0.9 + 10*2/7*100*0.9 = 557 (+90) MORA
            //                      4*2/3*10 + 2*2/6*10 + 10*2/7*10 = 61.9 (+10) BONUS
            //   MasterChef should have: 390 + 1000 - 557 - 100 = 733 (+100) MORA
            await advanceTimeAndBlock(9) // t+29
            await this.chef.connect(this.bob).withdraw(0, "5", { from: this.bob.address }) // t+30
            expect(await this.mora.totalSupply()).to.be.within(2000, 2100)
            expect(await this.mora.balanceOf(this.alice.address)).to.be.within(510 - this.tokenOffset, 600 + this.tokenOffset)
            expect(await this.bonusReward.balanceOf(this.alice.address)).to.be.within(56 - this.tokenOffset, 66 + this.tokenOffset)
            ...
            // Alice should have:   510 + 10*2/7*100*0.9 + 10*2/6.5*100*0.9 = 1044 (+90) MORA
            ...
            // Bob should have: 557 + 10*1.5/6.5*100*0.9 + 10*1.5/4.5*100*0.9 = 1064 (+90) MORA
            ...
            // Carol should have:   2*3/6*100*0.9 + 10*3/7*100*0.9 + 10*3/6.5*100*0.9 + 10*3/4.5*100*0.9 + 10*100*0.9 = 2391 (+90) MORA
        ...
        it("should give proper MORAs allocation to each pool", async function () {
            ...
            await this.chef.addPool("10", this.lp.address, this.rewarder.address, false) // t-51
            // Alice deposits 10 LPs at t+10
            await advanceTimeAndBlock(60) // t+9
            await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address }) // t+10
            // Add LP2 to the pool with allocation 2 at t+20
            await advanceTimeAndBlock(9) // t+19
            await this.chef.addPool("20", this.lp2.address, "0x0000000000000000000000000000000000000000", true) // t+20
            // Alice's pending reward should be:    10*100*0.9 = 900 (+90) MORA
            //                                      10*10 = 100 (+10) BONUS
            ...
            // Alice's pending reward should be:    900 + 5*1/3*100*0.9 = 1050 (+90) MORA
            ...
            // Alice's pending reward should be:    1050 + 5*1/3*100*0.9 = 1200 (+90) MORA
            //                                      150 + 5*10 = 200 (+10) BONUS
            // Bob's pending reward should be:  5*2/3*100*0.9 = 300 (+90) MORA
            //                                  0 BONUS
            ...
            // Alice should have:   1200 + 1*1/3*100*0.9 = 1230 (+90) MORA
            ...
            // Bob should have: 300 + 2*2/3*100*0.9 = 420 (+90) MORA
            //                  0 BONUS
