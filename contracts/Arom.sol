...
contract Arom is ERC20("Arom", "AROM"){
    ...
    function moraBalance(address _account) external view returns (uint256 moraAmount) {
        uint256 aromAmount = balanceOf(_account);
        uint256 totalArom = totalSupply();
        moraAmount = aromAmount.mul(mora.balanceOf(address(this))).div(totalArom);
    }

    function aromForMora(uint256 _aromAmount) external view returns (uint256 moraAmount) {
        uint256 totalArom = totalSupply();
        moraAmount = _aromAmount.mul(mora.balanceOf(address(this))).div(totalArom);
    }

    function moraForArom(uint256 _moraAmount) external view returns (uint256 aromAmount) {
        uint256 totalMora = mora.balanceOf(address(this));
        uint256 totalArom = totalSupply();
        if (totalArom == 0 || totalMora == 0) {
            aromAmount = _moraAmount;
        }
        else {
            aromAmount = _moraAmount.mul(totalArom).div(totalMora);
        }
    }
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s ) external {
        ...
        require(signatory != address(0), "AROM::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "AROM::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "AROM::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "AROM::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "AROM::_writeCheckpoint: block number exceeds 32 bits");
